...
function PicturesandSounds(acornsObject, data)
{
	...
	{ 
		...
		var iconSize = Math.ceil(imgView.width / points);
		if (iconSize<MIN_ICON_SIZE) iconSize = MIN_ICON_SIZE;
	...
	this.configurePlayPanel = function(panel) 
	{
		...
		var scaleFactor = picture.getScaleFactor();
		
		var div = document.createElement("div");
		div.style.width = width * scaleFactor / 100 + "px";
		div.style.height = height * scaleFactor / 100 + "px";
		div.style.left = (width - width * scaleFactor / 100)/2 + "px";
...
function CategoryRelatedPhrases(acornsObject, data, opt)
{
	...
	this.checkAnswer = function(text, indigenous, point, silent)
	{
		...
		var controlDistance = acorns.system.stringDistance(noControls.toLowerCase(), sentence.toLowerCase());
   ...
   var controlHandler = function(e, object, parent)
   {
		...
		switch (source.id)
		{
			...
			case "check":
				...
				if (plusCount==0)
						answer = object.checkAnswer(text, indigenous)[0];

			    if (answer === INCORRECT)
			    {
					object.feedback(parent, category, INCORRECT, "Incorrect answer, do you want to try again?",
						function(text) 
						{
							...
							{
								object.processNextSentence(parent, object);
							}
						}, false);
			    } 
				else if (answer == CLOSE)
			    {
					object.feedback(parent, category, CLOSE, "Not quite correct, check your answer", 
						function(e)
						{
							object.processNextSentence(parent, object);
						}, false);
...
function QuestionsandAnswers(acornsObject, data)
{
	...
	var makeCellPanel = function(cell, categoryChoice, cellChoice)
	{
		...
		span.innerHTML = categoryChoice.getSentence();
...
function MissingWord(acornsObject, data)
{
	...
	var makeButtonPanel = function()
	{
		...
		buttonPanel.style.marginTop = "50px";
    ...
    this.play = function(reset, changeLesson) 
	{
		...
		inputElement.value = "";
		if (select === "y") { sentence.applyFont(inputElement); }
	...
	this.configurePlayPanel = function(panel) 
	{
		...
		var buttonPanel = makeButtonPanel();
...
function StoryBook(acornsObject, data)
{
    ...
    this.configurePlayPanel = function(panel) 
	{
		...
		var lines = text.split("\n"), words, span, wordCount = 0;
		for (var lineNo=0; lineNo<lines.length; lineNo++)
		{ 
			words = lines[lineNo].split(" ");
			for (var wordNo=0; wordNo<words.length; wordNo++)
			{
				if (wordNo==0 && lineNo!=0) textDisplay.appendChild(document.createElement("br"));

				span = document.createElement("span");
				span.id = "" + wordCount++;
				acorns.system.applyFont(span, language);
				span.innerHTML = words[wordNo];
...
function HearandRespond(acornsObject, data)
{
    ...
    this.configurePlayPanel = function(panel) 
	{
		...
		textComponent.onkeydown = textHandler;
...
/***** The following section define the prototype classes for ACORNS classes *****/
...
function Lesson(acornsObject, lessonType, lessonNode)
{
	...
	this.parse = function(parseObject, node)	{  this.lessonObject.parse(parseObject, node); }
...
function Widgets(acornsObject)
{
	...
	this.configurePopupMenuItem = function(data, type, popupHandler)
	{
		...
		if (data.length>0)
		{
			...
			p.name = type;
			fontSize = Math.ceil(MENU_FONT * acorns.system.getFontRatio());
	...
	this.configurePopupMenu = function(info)
	{
		...
		for (var itemNo=0; itemNo<items.length; itemNo++)
		{
			...
			width = acorns.system.widthOfString(text[itemNo], spanFont);
			if (width > maxWidth) maxWidth = width;
		}
		popupMenu.style.lineHeight = spanFont + "px";
		if (items.length != 0)
		{
			var input = document.createElement("input");
			input.type = "button";

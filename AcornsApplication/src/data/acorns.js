...
function CategoryMultiplePictures(data)
{
	...
	{
		var layerList;
		var pictureList =  pictureList = this.getPictureList();
		...
		for (var p=0; p<pictureList.length; p++)
		{
			...
			excluded = layerList[layer-1].isExcluded();
			if (excluded)
				 audioList.push(null);
			else audioList.push(layerList[layer - 1].objectList);
		}
		return audioList;
	}
	...
	this.isPlayable = function(layer) 
    {
		...
		return total >= MIN_PICTURES;	   
    };
	
	this.getActivePictureList = function()
	{
		var pictureList = this.getPictureList();
		var activeList = [];
		var layer = lessonData.getLayer()
		for (var p=0; p<pictureList.length; p++)
		{
			layerList = pictureList[p].objectList;			
			excluded = layerList[layer-1].isExcluded();
			if (excluded)
				 activeList.push(null);
			else activeList.push(pictureList[p]);
		}
		return activeList;
	}
	...
	{ 
		...
		var activePictures = this.getActivePictureList();
		
		freeIndices = [];
		for (var i=0; i<activePictures.length; i++)
		{
			if (activePictures[i] && !selections.includes(i))
			{
				freeIndices.push(i);
			}
		}
	...
	this.selectPictures = function(numPictures)
	{
		...
		freeIndices = [];
		var pictureList = this.getActivePictureList();
		for (var i=0; i<pictureList.length; i++) 
		{ 
		    if (pictureList[i] && i!=pictureIndex)
			{
				freeIndices.push(i); 
			}
		}
		...
		var count = freeIndices.length;
		for (var i=1; i<numPictures; i++)
		{
			freeIndices[freeIndex] = freeIndices[count-- - 1];
			freeIndices.pop();
			
			freeIndex = Math.floor(Math.random() * count);
...
function Pictionary(acornsObject, data)
{
	...
	this.configurePlayPanel = function(panel) 
	{
		...
		var pictureList = category.getActivePictureList();
		var pictureCount = 0;
		for (var i=0; i<pictureList.length; i++)
		{
			if (pictureList[i])
		...
		if (columns > pictureCount) 
			rowWidth = pictureCount*(buttonSize + 2*BORDER) + (pictureCount - 1)*GAP;
...
function FlashCards(acornsObject, data)
{
 	...
 	var resetLesson = function()
	{
		...
		pictureList = category.getActivePictureList();
...
function Layer(nameL, valueL, languageL, alignL, excludedL)
{
   var name = (nameL==null) ? "" : nameL;
   var value = (valueL==null) ? 1 : valueL;
   var excluded = (excludedL) ? excludedL : "no";
...
{ 
	...
	this.parseMultiplePicturesCategory = function(acorns, lesson, lessonObject)
	{
	   ...
	   for (var imageNo=0; imageNo<images.length; imageNo++)
	   {
			...
			{
				var excluded = layers[layerNo].getAttribute("excluded");
				var value = layers[layerNo].getAttribute("value");
				layer  = new Layer(layers[layerNo].getAttribute("name"), 
					value, null, null, excluded);
